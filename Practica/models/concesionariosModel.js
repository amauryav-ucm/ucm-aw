...
function upsertConcesionarios(conn, arr, cb) {
    ...
    let inserted=0, updated=0, processed=0, errState=false;
    arr.forEach(c => {
      const id = c.id_concesionario;
      if (!id) { processed++; if (processed===arr.length) cb(null,{inserted,updated}); return; }
      conn.query("SELECT 1 FROM concesionarios WHERE id_concesionario=?", [id], (err, rows) => {
        if (err && !errState) { errState=true; return cb(err); }
        if (rows && rows.length) {
          conn.query(
            ...
            err2 => { if (err2 && !errState) { errState=true; return cb(err2);} updated++; processed++; if (processed===arr.length) cb(null,{inserted,updated}); }
          );
        } else {
          conn.query(
            ...
            err2 => { if (err2 && !errState) { errState=true; return cb(err2);} inserted++; processed++; if (processed===arr.length) cb(null,{inserted,updated}); }
          );
        }
      });
    });
